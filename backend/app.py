...
def import_catalog():
    ...
    urls = payload.get('urls') or DEFAULT_FEEDS

    session = get_session()
    try:
        summary = import_shopify_feeds(session, urls)
...
def search():
    ...
    try:
        if has_file:
            ...
            results = _search_catalog_by_image(session, filepath)
            return jsonify(
                {
                    'success': True,
                    'results': results,
...
def _search_catalog_by_image(session, filepath: str, limit: int = 6) -> list[dict]:
...
def _fallback_catalog(session, limit: int = 6) -> list[dict]:
    products = (
        session.query(CatalogProduct)
        .filter(CatalogProduct.is_active.is_(True))
